...
export async function generateTourConfirmation(params: TourConfirmationParams): Promise<string> {
  const { leadName, venueName, personaName, scheduledAt, durationMinutes, locationNotes, requestId } = params
  ...
  const response = await withAnthropicRetry(
    ...
    { agent: 'tour-scheduler', requestId, model: MODEL }
  )
...
export async function generateTourReminder(params: {
  ...
  const { leadName, venueName, personaName, scheduledAt, hoursUntil, requestId } = params
  const firstName = leadName.split(' ')[0]
  const formattedDate = format(new Date(scheduledAt), "EEEE, MMMM d 'at' h:mm a")
